      ...
      <div className="mb-8 flex flex-col md:flex-row md:items-center justify-between border-b border-dark-border pb-6 gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-2 font-medium">
            Lucky analisando{" "}
            <span className="text-gray-300">notícias, tabus e desfalques</span>{" "}
            em tempo real.
          </p>
        </div>
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-20 gap-6">
          <div className="relative">
            ...
            <Bot
              size={24}
              className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-neon"
            />
          </div>
        ...
        <div className="text-center py-20 bg-dark-card rounded-3xl border border-dashed border-dark-border">
          ...
          <p className="text-gray-400 font-bold uppercase text-sm">
            Nenhuma zebra detectada agora.
          </p>
          <p className="text-gray-600 text-xs mt-1">
            Os favoritos estão sob controle total.
          </p>
        </div>
      ) : (
        <div className="flex flex-col gap-8">
          {jogos.map((jogo) => (
            <div key={jogo.id} className="relative group">
              ...
              <Link
                to={`/jogo/${jogo.id}`}
                className="block bg-[#121212] border border-dark-border rounded-4xl p-6 transition-all hover:border-neon/50 hover:shadow-[0_10px_40px_rgba(0,0,0,0.6)] relative overflow-hidden"
              >
                {/* Placar e Tempo Real */}
                ...
                <div className="grid grid-cols-3 items-center mb-8 px-2">
                  <div className="text-center">
                    ...
                    <p className="font-bold text-gray-400 truncate text-sm md:text-base">
                      {jogo.favorito}
                    </p>
                  </div>
                  <div className="bg-black py-2 rounded-2xl border border-white/5 shadow-inner">
                    <p className="font-heading text-3xl text-center text-white tracking-widest">
                      {jogo.placar}
                    </p>
                  </div>
                  <div className="text-center">
                    <p className="text-[10px] font-black text-neon uppercase mb-1">
                      Zebra
                    </p>
                  ...
                  <div>
                    ...
                    <div className="h-1.5 bg-gray-800 rounded-full overflow-hidden">
                      <div
                        ...
                        style={{
                          width: `${Math.min(jogo.ip_favorito * 50, 100)}%`,
                        }}
                      />
                    </div>
                  </div>
                  <div>
                    ...
                    <div className="h-1.5 bg-gray-800 rounded-full overflow-hidden">
                      <div
                        ...
                        style={{
                          width: `${Math.min(jogo.ip_zebra * 50, 100)}%`,
                        }}
                      />
                    </div>
                  </div>
