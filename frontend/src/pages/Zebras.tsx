...
import { Ticket, TrendingUp, AlertTriangle, ChevronRight } from 'lucide-react';
    ...
    <div className="flex flex-col w-full min-h-[calc(100dvh-80px)] px-4 py-6 md:py-10 pb-28 md:pb-10 animate-fade-in-up max-w-3xl mx-auto">
      ...
      <div className="mb-6 flex items-center justify-between border-b border-dark-border pb-4">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-1">Azarões com alto Índice de Pressão (IP).</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="flex flex-col gap-4">
          {[1, 2].map((i) => (
            <div key={i} className="h-40 bg-dark-card rounded-xl animate-pulse border border-dark-border" />
          ))}
        </div>
      ) : (
        // Lista de Zebras
            ...
            >
              ...
              <div className="flex items-center justify-between relative z-10 bg-dark-bg/50 rounded-lg p-3 border border-dark-border/50">
                <div className="flex-1 text-right">
                  <p className="font-semibold text-sm md:text-base text-gray-500 uppercase tracking-wider">Favorito</p>
                  <p className="font-bold text-base md:text-xl text-gray-400 truncate strike">{jogo.favorito}</p>
                  <p className="text-xs text-gray-500 mt-1">IP: {jogo.ip_favorito.toFixed(2)}</p>
                </div>
                ...
                <div className="flex-1 text-left">
                  <p className="font-semibold text-sm md:text-base text-neon uppercase tracking-wider flex items-center gap-1">Zebra <Ticket size={14}/></p>
                  <p className="font-bold text-base md:text-xl text-white truncate">{jogo.zebra}</p>
                  <p className="text-xs text-neon font-bold mt-1">IP: {jogo.ip_zebra.toFixed(2)}</p>
                </div>
              </div>
              ...
              <div className="mt-4 pt-3 border-t border-dark-border flex justify-center items-center text-xs font-bold text-gray-400 group-hover:text-neon transition-colors uppercase tracking-widest">
                Analisar Oportunidade <ChevronRight size={14} className="ml-1" />
              </div>
            </Link>
          ))}
        </div>
