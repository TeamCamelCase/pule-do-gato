  ...
  return (
    <div className="flex flex-col w-full max-w-2xl mx-auto min-h-[calc(100dvh-80px)] px-4 mt-6 md:mt-10 pb-32 md:pb-16 animate-fade-in-up">
      ...
      <div className="flex items-center gap-3 border-b border-dark-border pb-5 mb-8">
        ...
        <div>
          <h2 className="text-2xl font-bold font-heading tracking-wide uppercase">Olheiro AI: Mestre Tático</h2>
          <p className="text-sm text-gray-400 mt-1">Olá! Eu sou o assistente do Pule do Gato. O que vamos analisar hoje?</p>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col gap-8">
        ...
        <div className="flex flex-col items-start gap-3 w-full animate-fade-in-up">
          <div className="flex items-center gap-2">
            ...
            <span className="font-bold text-sm text-gray-300">Olheiro AI</span>
          </div>
          ...
          <div className="flex flex-wrap gap-2 mt-2 ml-4">
            <button 
              onClick={() => { setEsporte('futebol'); setPasso(1); }} 
              className={`px-5 py-2.5 rounded-full text-sm font-bold flex items-center gap-2 border transition-all ${esporte === 'futebol' ? 'bg-neon text-dark-bg border-neon shadow-[0_0_15px_rgba(204,255,0,0.2)]' : 'bg-dark-bg text-white border-dark-border hover:border-gray-500 hover:bg-dark-card'}`}
            >
              Futebol ⚽
            </button>
            <button 
              onClick={() => { setEsporte('basquete'); setPasso(1); }}
              className={`px-5 py-2.5 rounded-full text-sm font-bold flex items-center gap-2 border transition-all ${esporte === 'basquete' ? 'bg-neon text-dark-bg border-neon shadow-[0_0_15px_rgba(204,255,0,0.2)]' : 'bg-dark-bg text-white border-dark-border hover:border-gray-500 hover:bg-dark-card'}`}
            >
              Basquete 🏀
            </button>
          </div>
        ...
        {passo >= 1 && esporte && (
          <div className="flex flex-col items-start gap-3 w-full animate-fade-in-up delay-100">
            ...
            <div className="bg-dark-card p-4 rounded-xl rounded-bl-none border border-dark-border text-white text-base max-w-[85%]">
              Ótima escolha! Qual campeonato você quer analisar as estatísticas táticas e o Índice de Pressão AI?
            </div>
            ...
            <div className="flex flex-wrap gap-2 mt-2 ml-4 max-w-[90%]">
              {opcoes[esporte as keyof typeof opcoes].map((item) => (
                <button 
                  key={item.id}
                  onClick={() => { setLiga(item); setPasso(2); }}
                  className={`px-5 py-2.5 rounded-full text-sm font-bold flex items-center gap-2 border transition-all ${liga?.id === item.id ? 'bg-neon text-dark-bg border-neon shadow-[0_0_15px_rgba(204,255,0,0.2)]' : 'bg-dark-bg text-white border-dark-border hover:border-gray-500 hover:bg-dark-card'}`}
                >
        ...
        {passo >= 2 && liga && (
          <div className="flex flex-col items-start gap-3 w-full animate-fade-in-up delay-200 mb-8 pb-8 border-b border-dark-border">
            ...
            <div className="bg-dark-card p-4 rounded-xl rounded-bl-none border border-dark-border text-white text-base max-w-[85%] relative overflow-hidden">
              ...
              Pronto! O Radar AI está monitorando a **{liga.nome}** {liga.icone}. Eu separei as melhores oportunidades táticas e os jogos que estão **"Fervendo"** no Índice de Pressão (IP). <br className="hidden md:block" /> Vamos lá?
            </div>
            ...
            <button onClick={resetChat} className="text-xs text-gray-500 hover:text-white mt-4 ml-4">Reiniciar Chat</button>
          </div>
        )}
      </div>
