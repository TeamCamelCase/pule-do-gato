import { Activity, AlertCircle, ChevronRight, Flame } from "lucide-react";
...
export default function FeedQuente() {
  ...
  useEffect(() => {
    ...
    const buscarJogos = async () => {
      try {
        ...
        console.error(err);
        setErro(
          "Não foi possível conectar ao Radar AI. Verifique se o backend está rodando."
        );
        ...
        setJogos([
          {
            id: "11648148",
            time_casa: "São Paulo U20",
            time_fora: "Ferroviária U20",
            placar: "1-0",
            tempo: "57",
            ip_casa: 0.44,
            ip_fora: 0.25,
  ...
  return (
    <div className="flex flex-col align-middle w-full min-h-[calc(100dvh-80px)] px-4 py-6 md:py-10 pb-28 md:pb-10 animate-fade-in-up md:w-300 lg:w-300 m-auto">
      {/* Cabeçalho da Página */}
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-1">
            Jogos filtrados pelo Índice de Pressão AI
          </p>
        </div>
        ...
        <div className="bg-red-900/20 border border-red-500/50 text-red-200 p-4 rounded-lg mb-6 flex items-start gap-3 text-sm">
          ...
          <p>
            ...
            <span className="text-gray-400 text-xs">
              Exibindo dados simulados para teste.
            </span>
          </p>
        </div>
      )}
      ...
      {loading ? (
        ...
        <div className="flex flex-col gap-4">
          {[1, 2, 3].map((i) => (
            <div
              key={i}
              className="h-32 bg-dark-card rounded-xl animate-pulse border border-dark-border"
            />
          ))}
        </div>
      ) : (
          ...
          {jogos.map((jogo) => (
            <Link
              ...
              className={`block bg-dark-card border rounded-xl p-4 transition-all hover:border-gray-500 active:scale-[0.98] relative overflow-hidden group ${
                jogo.alerta === "CRÍTICO"
                  ? "border-neon/50 shadow-[0_0_15px_rgba(204,255,0,0.1)]"
                  : "border-dark-border"
              }`}
            >
              ...
              <div className="flex justify-between items-center mb-4">
                ...
                {(jogo.ip_casa || jogo.ip_fora) && (
                  <span className="flex items-center gap-1 text-xs font-bold text-gray-300">
                    ...
                    />
                    IP Max:{" "}
                    {Math.max(jogo.ip_casa || 0, jogo.ip_fora || 0).toFixed(2)}
                  </span>
                )}
              </div>
